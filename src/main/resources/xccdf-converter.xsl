...
<!-- Adapted from XCCDF 1.1.4 to 1.2 converter found here: http://scap.nist.gov/specifications/xccdf/index.html#resource-1.2 -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cdf="http://checklists.nist.gov/xccdf/1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cpe="http://cpe.mitre.org/dictionary/2.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dsig="http://www.w3.org/2000/09/xmldsig#" xmlns="http://checklists.nist.gov/xccdf/1.2" exclude-result-prefixes="xs cdf" version="2.0">
	...
	<!-- Include user defined namespace -->
  <xsl:param name="local_var_reverseDNSnamespace"/> <!-- Your organization's reverse-DNS format namespace -->
  <xsl:param name="schema_location"/> 	   <!-- The path to and name of the XCCDF schema -->
  <xsl:template match="comment()">
		...
		<xsl:choose>
			<xsl:when test="namespace-uri(.)='http://checklists.nist.gov/xccdf/1.1'">
				<xsl:element name="{local-name()}" namespace="http://checklists.nist.gov/xccdf/1.2">
					<xsl:copy-of select="@*"/>
					<xsl:if test="name(.)!='notice' and name(.)!='platform'">
						<xsl:if test="@id">
							<xsl:attribute name="id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_benchmark_', @id)"/>
						</xsl:if>
						<xsl:if test="@value-id">
							<xsl:attribute name="value-id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_value_', @value-id)"/>
						</xsl:if>
						<xsl:if test="@idref">
							<xsl:choose>
								<xsl:when test="name()='requires'">
									<xsl:attribute name="idref"><xsl:call-template name="Requires"><xsl:with-param name="string" select="string(@idref)"/></xsl:call-template></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:variable name="idref-value" select="string(@idref)"/>
									<xsl:choose>
										<xsl:when test="//cdf:Group[@id=$idref-value]">
											<xsl:attribute name="idref" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_group_', $idref-value)"/>
										</xsl:when>
										<xsl:when test="//cdf:Rule[@id=$idref-value]">
											<xsl:attribute name="idref" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_rule_', $idref-value)"/>
										</xsl:when>
										<xsl:when test="//cdf:Value[@id=$idref-value]">
											<xsl:attribute name="idref" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_value_', $idref-value)"/>
										</xsl:when>
										<xsl:when test="//cdf:Profile[@id=$idref-value]">
											<xsl:attribute name="idref" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_profile_', $idref-value)"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:attribute name="idref" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', $idref-value)"/>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:otherwise>
							</xsl:choose>
			...
			<xsl:when test="contains($string, ' ')">
				<xsl:choose>
					<xsl:when test="//cdf:Group[@id=substring-before($string, ' ')]">
						<xsl:value-of select="concat('xccdf_', $local_var_reverseDNSnamespace, '_group_', substring-before($string, ' '))"/>
					</xsl:when>
					<xsl:when test="//cdf:Rule[@id=substring-before($string, ' ')]">
						<xsl:value-of select="concat('xccdf_', $local_var_reverseDNSnamespace, '_rule_', substring-before($string, ' '))"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', substring-before($string, ' '))"/>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:text> </xsl:text>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="//cdf:Group[@id=$string]">
						<xsl:value-of select="concat('xccdf_', $local_var_reverseDNSnamespace, '_group_', $string)"/>
					</xsl:when>
					<xsl:when test="//cdf:Rule[@id=$string]">
						<xsl:value-of select="concat('xccdf_', $local_var_reverseDNSnamespace, '_rule_', $string)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', $string)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="cdf:Benchmark">
		...
		<xsl:element name="Benchmark">
			...
			<xsl:attribute name="xsi:schemaLocation"><xsl:call-template name="SchemaLoc"><xsl:with-param name="string" select="string(@xsi:schemaLocation)"/></xsl:call-template></xsl:attribute>
			<!-- Copy child elements -->
			...
			<xsl:when test="contains($string, ' ')">
				<xsl:choose>
					<xsl:when test="substring-before($string, ' ')='http://checklists.nist.gov/xccdf/1.1'">
						<xsl:value-of select="concat('http://checklists.nist.gov/xccdf/2.0 ', $schema_location)"/>
	...
	<xsl:template match="cdf:Group">
		...
		<xsl:element name="Group">
			...
			<xsl:copy-of select="@*"/>
			<!-- Update id attribute -->
			<xsl:attribute name="id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_group_', @id)"/>
			...
			<xsl:if test="@extends">
				...
				<xsl:choose>
					<xsl:when test="//cdf:Group[@id=$extends-value]">
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_group_', @extends)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', @extends)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<!-- Copy child elements -->
	...
	<xsl:template match="cdf:Rule">
		...
		<xsl:element name="Rule">
			...
			<xsl:copy-of select="@*"/>
			<!-- Update id attribute -->
			<xsl:attribute name="id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_rule_', @id)"/>
			<!-- If present, update extends attribute -->
			<xsl:if test="@extends">
				<xsl:variable name="extends-value" select="string(@extends)"/>
				<xsl:choose>
					<xsl:when test="//cdf:Rule[@id=$extends-value]">
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_rule_', @extends)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', @extends)"/>
					</xsl:otherwise>
	...
	<xsl:template match="cdf:Value">
		...
		<xsl:element name="Value">
			...
			<xsl:copy-of select="@*"/>
			<!-- Update id attribute -->
			<xsl:attribute name="id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_value_', @id)"/>
			<!-- If present, update extends attribute -->
			<xsl:if test="@extends">
				<xsl:variable name="extends-value" select="string(@extends)"/>
				<xsl:choose>
					<xsl:when test="//cdf:Value[@id=$extends-value]">
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_value_', @extends)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', @extends)"/>
					</xsl:otherwise>
	...
	<xsl:template match="cdf:Profile">
		...
		<xsl:element name="Profile">
			...
			<xsl:copy-of select="@*"/>
			<!-- Update id attribute -->
			<xsl:attribute name="id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_profile_', @id)"/>
			<!-- If present, update extends attribute -->
			<xsl:if test="@extends">
				<xsl:variable name="extends-value" select="string(@extends)"/>
				<xsl:choose>
					<xsl:when test="//cdf:Profile[@id=$extends-value]">
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_profile_', @extends)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="extends" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_?????_', @extends)"/>
					</xsl:otherwise>
	...
	<xsl:template match="cdf:TestResult">
		...
		<xsl:element name="TestResult">
			...
			<xsl:copy-of select="@*"/>
			<!-- Update id attribute -->
			<xsl:attribute name="id" select="concat('xccdf_', $local_var_reverseDNSnamespace, '_testresult_', @id)"/>
			<!-- Copy child elements -->
			<xsl:apply-templates select="*|comment()"/>
		</xsl:element>
	</xsl:template>
